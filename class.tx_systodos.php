 ...
 * @author Christian Jul Jensen <christian(at)bee3(dot)com>
 */
...
class tx_systodos extends mod_user_task {
	...
	function todos_link($str, $id) {
		$str = '<a href="index.php?SET[function]=tx_systodos&sys_todos_uid='.$id.'" onClick="this.blur();">'.$str.'</a>';
		return $str;
	} //todos_link
	/**
	...
	function renderTasks() {
		...
		if(!$tUid && !is_string(t3lib_div::_GP('action'))) {
			...
			if(t3lib_div::_GP('workflow_type')) {
				...
				return $this->getCSS().$this->pObj->doc->getDynTabMenu($menuItems,'tx_systodos',0);
